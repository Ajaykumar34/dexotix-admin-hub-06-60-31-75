...
-- Fix the jsonb_array_length type issue in booking triggers

-- Update the trigger function to properly handle JSON vs JSONB types
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
    ...
    IF NEW.seat_numbers IS NOT NULL THEN
      -- Convert to JSONB if it's JSON type
      seat_numbers_data := CASE 
        WHEN pg_typeof(NEW.seat_numbers) = 'json'::regtype THEN NEW.seat_numbers::jsonb
        ELSE NEW.seat_numbers
      END;
      
      -- Calculate quantity from seat_numbers array
      IF seat_numbers_data IS NOT NULL AND jsonb_typeof(seat_numbers_data) = 'array' THEN
        SELECT COALESCE(SUM(COALESCE((item->>'quantity')::integer, 1)), 0)
        INTO calculated_quantity
        FROM jsonb_array_elements(seat_numbers_data) AS item;
