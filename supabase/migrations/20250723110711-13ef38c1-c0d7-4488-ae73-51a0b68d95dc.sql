-- Fix availability update issues by ensuring triggers are properly working

-- First, let's check and fix the trigger function for occurrence ticket categories
...
-- Function to recalculate all event seat pricing availability
CREATE OR REPLACE FUNCTION public.recalculate_event_availability(event_id_param UUID)
RETURNS void
LANGUAGE plpgsql
AS $function$
BEGIN
  -- Recalculate availability for event_seat_pricing
  UPDATE event_seat_pricing 
  SET available_tickets = (
    SELECT GREATEST(0, esp.total_tickets - COALESCE(SUM(b.quantity), 0))
    FROM event_seat_pricing esp
    LEFT JOIN bookings b ON b.event_id = esp.event_id 
      AND b.status = 'Confirmed'
      AND b.event_occurrence_id IS NULL  -- Only count non-recurring bookings
    WHERE esp.id = event_seat_pricing.id
    GROUP BY esp.id, esp.total_tickets
  ),
  updated_at = NOW()
  WHERE event_id = event_id_param;
  
  RAISE NOTICE 'Recalculated availability for event %', event_id_param;
END;
$function$;