...
DROP TABLE IF EXISTS global_seat_categories CASCADE;
...
-- Create seat_categories table (linked to events)
...
-- Create global_seat_categories table
CREATE TABLE global_seat_categories (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL,
  description TEXT,
  color TEXT NOT NULL DEFAULT '#4ECDC4',
...
ALTER TABLE global_seat_categories ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Allow public read access" ON global_seat_categories FOR SELECT USING (true);
...
CREATE POLICY "Allow all for admins" ON global_seat_categories FOR ALL USING (true) WITH CHECK (true);
...
-- Insert sample global seat categories
INSERT INTO global_seat_categories (name, description, color, base_price) VALUES
('General', 'Standard seating', '#4ECDC4', 500),
('VIP', 'Premium VIP seating', '#FFD700', 1000),
('Premium', 'Premium seating', '#FF6B6B', 750),
('Balcony', 'Balcony seating', '#45B7D1', 300),
('Box', 'Private box seating', '#96CEB4', 1200),
('Orchestra', 'Orchestra level seating', '#FFEAA7', 800),
('Mezzanine', 'Mezzanine level seating', '#DDA0DD', 600),
('Economy', 'Budget-friendly seating', '#98D8C8', 250);

-- Insert sample events using separate INSERT statements
INSERT INTO events (name, description, category, start_datetime, end_datetime, sale_start, sale_end, poster, artist_name, is_featured, is_regular)
VALUES 
('Sample Concert Event', 'An amazing live concert featuring top artists in a spectacular venue.', 'Concert', 
 NOW() + INTERVAL '30 days', NOW() + INTERVAL '30 days' + INTERVAL '3 hours', 
 NOW(), NOW() + INTERVAL '29 days', 
 'https://example.com/poster1.jpg', 'Sample Artist', true, false);

INSERT INTO events (name, description, category, start_datetime, end_datetime, sale_start, sale_end, poster, artist_name, is_featured, is_regular)
VALUES 
('Theater Play Event', 'A captivating theater performance that will leave you mesmerized.', 'Theater', 
 NOW() + INTERVAL '45 days', NOW() + INTERVAL '45 days' + INTERVAL '2 hours', 
 NOW(), NOW() + INTERVAL '44 days', 
 'https://example.com/poster2.jpg', 'Theater Company', false, true);

INSERT INTO events (name, description, category, start_datetime, end_datetime, sale_start, sale_end, poster, artist_name, is_featured, is_regular)
VALUES 
('Sports Championship', 'An exciting sports event featuring top teams competing for the championship.', 'Sports', 
 NOW() + INTERVAL '60 days', NOW() + INTERVAL '60 days' + INTERVAL '4 hours', 
 NOW(), NOW() + INTERVAL '59 days', 
 'https://example.com/poster3.jpg', 'Championship Teams', true, true);

-- Insert sample carousel slides
INSERT INTO carousel_slides (title, subtitle, description, image, button_text, button_link, city, sort_order) VALUES
('Welcome to Event Hub', 'Your Premier Event Destination', 'Discover amazing events happening in your city', 'https://example.com/carousel1.jpg', 'Explore Events', '/events', 'Mumbai', 1),
('Live Concerts', 'Experience Music Like Never Before', 'Book tickets for the hottest concerts in town', 'https://example.com/carousel2.jpg', 'Book Now', '/events?category=concert', 'Delhi', 2),
('Theater Shows', 'Drama, Comedy, and More', 'Immerse yourself in captivating theatrical performances', 'https://example.com/carousel3.jpg', 'View Shows', '/events?category=theater', 'Bangalore', 3);
