...
-- Enhanced fix for category-specific deduction that properly handles the data format
...
BEGIN
  ...
  IF TG_OP = 'INSERT' AND NEW.event_occurrence_id IS NULL AND NEW.occurrence_ticket_category_id IS NULL THEN
    ...
    -- Try to determine the specific seat category from seat_numbers with multiple strategies
    IF NEW.seat_numbers IS NOT NULL THEN
      BEGIN
        -- Convert to JSONB if it's JSON
        IF pg_typeof(NEW.seat_numbers) = 'json'::regtype THEN
        ...
        IF jsonb_typeof(seat_numbers_data) = 'array' AND jsonb_array_length(seat_numbers_data) > 0 THEN
          ...
          RAISE NOTICE 'INSERT trigger: Extracted category name from booking: %', category_name_from_booking;
          
          -- Find the matching seat category ID
          IF category_name_from_booking IS NOT NULL THEN
            ...
            WHERE sc.event_id = NEW.event_id 
              AND sc.name = category_name_from_booking
        ...
        END IF;
      EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'INSERT trigger: Error processing seat_numbers for category lookup: %', SQLERRM;
        target_seat_category_id := NULL;
      END;
    END IF;
    
    -- If we found a specific category, update only that category
    IF target_seat_category_id IS NOT NULL THEN
      ...
      RAISE NOTICE 'INSERT trigger: Updated SPECIFIC category % (%) for non-recurring event', target_seat_category_id, category_name_from_booking;
    ELSE
      -- If no specific category found, try to find a default "General" category
      SELECT sc.id INTO target_seat_category_id
      FROM seat_categories sc
      WHERE sc.event_id = NEW.event_id 
        AND sc.name = 'General'
      ...
      IF target_seat_category_id IS NOT NULL THEN
        ...
        RAISE NOTICE 'INSERT trigger: Updated General category % for non-recurring event (fallback)', target_seat_category_id;
      ELSE
        -- Last resort: update all categories (old behavior)
        UPDATE event_seat_pricing 
        SET available_tickets = GREATEST(0, available_tickets - COALESCE(NEW.quantity, 1)),
            updated_at = NOW()
        WHERE event_id = NEW.event_id AND is_active = true;
        
        RAISE NOTICE 'INSERT trigger: Updated ALL categories for non-recurring event (last resort fallback)';
      END IF;
    END IF;
  ...
  IF TG_OP = 'DELETE' AND OLD.event_occurrence_id IS NULL AND OLD.occurrence_ticket_category_id IS NULL THEN
    ...
    IF OLD.seat_numbers IS NOT NULL THEN
      BEGIN
        ...
        END IF;
      EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'DELETE trigger: Error processing seat_numbers for category lookup: %', SQLERRM;
        target_seat_category_id := NULL;
      END;
    END IF;
    
    -- If we found a specific category, restore only that category
    IF target_seat_category_id IS NOT NULL THEN
      UPDATE event_seat_pricing 
      ...
      RAISE NOTICE 'DELETE trigger: Restored SPECIFIC category % for non-recurring event', target_seat_category_id;
    ELSE
      ...
      IF target_seat_category_id IS NOT NULL THEN
        ...
        RAISE NOTICE 'DELETE trigger: Restored General category % for non-recurring event (fallback)', target_seat_category_id;
      ELSE
        -- Last resort: restore all categories
        UPDATE event_seat_pricing 
        SET available_tickets = LEAST(total_tickets, available_tickets + COALESCE(OLD.quantity, 1)),
            updated_at = NOW()
        WHERE event_id = OLD.event_id AND is_active = true;
        
        RAISE NOTICE 'DELETE trigger: Restored ALL categories for non-recurring event (last resort fallback)';
      END IF;
    END IF;
      ...
      IF ticket_change != 0 THEN
        ...
        IF NEW.seat_numbers IS NOT NULL THEN
          BEGIN
            ...
            END IF;
          EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'UPDATE trigger: Error processing seat_numbers for category lookup: %', SQLERRM;
            target_seat_category_id := NULL;
          END;
        END IF;
        ...
        IF target_seat_category_id IS NOT NULL THEN
          ...
          RAISE NOTICE 'UPDATE trigger: Updated SPECIFIC category % for non-recurring event', target_seat_category_id;
        ELSE
          -- Try to find General category
          SELECT sc.id INTO target_seat_category_id
          FROM seat_categories sc
          ...
          IF target_seat_category_id IS NOT NULL THEN
            ...
            RAISE NOTICE 'UPDATE trigger: Updated General category % for non-recurring event (fallback)', target_seat_category_id;
          ELSE
            -- Last resort: update all categories
            UPDATE event_seat_pricing 
            SET available_tickets = GREATEST(0, LEAST(total_tickets, available_tickets + ticket_change)),
                updated_at = NOW()
            WHERE event_id = NEW.event_id AND is_active = true;
            
            RAISE NOTICE 'UPDATE trigger: Updated ALL categories for non-recurring event (last resort fallback)';
          END IF;
        END IF;
      END IF;
    END IF;
