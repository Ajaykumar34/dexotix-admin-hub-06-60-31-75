...
-- Fix the trigger to ensure non-recurring events also only deduct from the specific category
  ...
  IF TG_OP = 'INSERT' AND NEW.event_occurrence_id IS NULL AND NEW.occurrence_ticket_category_id IS NULL THEN
    ...
    IF NEW.seat_numbers IS NOT NULL THEN
      BEGIN
        ...
        -- Get the first seat category from seat_numbers
        IF jsonb_typeof(seat_numbers_data) = 'array' AND jsonb_array_length(seat_numbers_data) > 0 THEN
          ...
          WHERE sc.event_id = NEW.event_id 
            AND sc.name = (seat_numbers_data->0->>'seat_category')
            AND sc.is_active = true
          LIMIT 1;
        END IF;
      EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'INSERT trigger: Error processing seat_numbers for category lookup';
    ...
    IF target_seat_category_id IS NOT NULL THEN
      ...
      RAISE NOTICE 'INSERT trigger: Updated SPECIFIC category % for non-recurring event', target_seat_category_id;
    ELSE
      -- Fallback: update all categories (old behavior)
      UPDATE event_seat_pricing 
      SET available_tickets = GREATEST(0, available_tickets - COALESCE(NEW.quantity, 1)),
          updated_at = NOW()
      WHERE event_id = NEW.event_id AND is_active = true;
      
      RAISE NOTICE 'INSERT trigger: Updated ALL categories for non-recurring event (fallback)';
    END IF;
  ...
  IF TG_OP = 'DELETE' AND OLD.event_occurrence_id IS NULL AND OLD.occurrence_ticket_category_id IS NULL THEN
    ...
    IF OLD.seat_numbers IS NOT NULL THEN
      BEGIN
        ...
        END IF;
      EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'DELETE trigger: Error processing seat_numbers for category lookup';
        target_seat_category_id := NULL;
      END;
    END IF;
    ...
    ELSE
      -- Fallback: restore all categories (old behavior)
      UPDATE event_seat_pricing 
      SET available_tickets = LEAST(total_tickets, available_tickets + COALESCE(OLD.quantity, 1)),
          updated_at = NOW()
      WHERE event_id = OLD.event_id AND is_active = true;
      
      RAISE NOTICE 'DELETE trigger: Restored ALL categories for non-recurring event (fallback)';
    END IF;
      ...
      IF ticket_change != 0 THEN
        ...
        IF NEW.seat_numbers IS NOT NULL THEN
          BEGIN
            ...
            END IF;
          EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'UPDATE trigger: Error processing seat_numbers for category lookup';
            target_seat_category_id := NULL;
          END;
        END IF;
        ...
        ELSE
          ...
          RAISE NOTICE 'UPDATE trigger: Updated ALL categories for non-recurring event (fallback)';
        END IF;
      END IF;
    END IF;
